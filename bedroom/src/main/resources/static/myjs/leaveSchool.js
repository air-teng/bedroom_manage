 ...
 * 提交按钮生成申请方案
 */
function leaveSchoolProgram(){
	var AAA=confirm("确认提交?");
	if(!AAA){  return ;  };
	//获取前端时间
	var  account=$("#student-account").val();
	var sTime=datetimeLocalToDate($("#leave-start-time").val());
	var eTime=datetimeLocalToDate($("#leave-end-time").val());
	var reason=$("#leave-apply-reason").val();
	//判断前端输入的数据是否正确
...
//				"scrollX":true,
			    ...
			    //ajax实现数据的传递的再现
			    ajax:{
	                url: "/affair/showApplyLeaveInfo",//修改路径即可
	                type: 'POST',
