...
$(function() {
	//展示学生离校申请信息
	showApprovalApplyLeaveInfo();
	
	//展示学生离校审批结果信息
	showApprovalResultLeaveSchool();
 ...
 * @returns
 */
function showApprovalApplyLeaveInfo(){
	$("#approval-apply-leave").DataTable(
			{
...
//				"scrollX":true,
			    ...
			    //ajax实现数据的传递的再现
			    ajax:{
	                url: "/counselor/showApprovalLeaveInfo",//修改路径即可
	                type: 'POST',
			    ...
			    columns: [　//这个是显示到界面上的个数据　格式为 {data:'显示的字段名'}
			        ...
			        {"data":'affairId',
			        	render:function(data){
			        		return '<input type="button" value="同意" onclick="agreeLeaveSchool('+data+')"></input>'+
			        		'&nbsp;&nbsp;<input type="button" value="拒绝" onclick="refuseLeaveSchool('+data+')"></input>';
			        	}
			        }
 ...
 * 辅导员同意申请;  操作为同意
 */
function agreeLeaveSchool(data){
	var AAA=confirm("确认提交?");
	if(!AAA){  return ;  };
	//ajax请求,执行操作
	$.ajax({
		type: "POST",
		url: "/counselor/counselorAgreeLeaveSchool",
 ...
 * 辅导员同意申请;  操作为同意
 */
function refuseLeaveSchool(data){
	var AAA=confirm("确认提交?");
	if(!AAA){  return ;  };
	//ajax请求,执行操作
	$.ajax({
		type: "POST",
		url: "/counselor/counselorRefuseLeaveSchool",
		data: {"affairId" : parseInt(data) },
		success: function(result){
			if(result.status=="true"){ 
				alert("操作成功"); 
 ...
 * @returns
 */
function showApprovalResultLeaveSchool(){
	$("#show-approval-result-leave").DataTable(
			{
				"aLengthMenu": [ 5, 10,20],//设置显示条数
//				"scrollX":true,
				"responsive": false,
			    ...
			    //ajax实现数据的传递的再现
			    ajax:{
	                url: "/counselor/showApprovalResultLeaveSchool",//修改路径即可
	                type: 'POST',
	                //用于处理服务器端返回的数据。 dataSrc是DataTable特有的
	                dataSrc: function (myJson) {
	                    if (myJson.timeout) {
