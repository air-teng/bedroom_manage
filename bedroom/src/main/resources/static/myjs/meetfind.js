...
function initAllToReplyTable(){
	$("#all-to-reply-table").DataTable({
	        ...
	        ajax:{
                url: "/affair/getAllChangeToReply",//修改路径即可
                type: 'GET',
...
function replyAffairModal(row){
	...
	$("input[name='curBedroomName']").val(row.curBedroomName);
	$("input[name='targetBedroomName']").val(row.targetBedroomName);
